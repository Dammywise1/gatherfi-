...
# Builds EVERYTHING needed for your dApp
# Based on your lib.rs smart contract
# ============================================
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     GATHERFI - COMPLETE FRONTEND GENERATOR                   ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
...
rm -rf src app components hooks utils contexts lib public
rm -f .env.local next.config.js tailwind.config.ts tsconfig.json package.json package-lock.json
...
# ============================================
echo -e "${GREEN}3. Installing Solana/Anchor dependencies...${NC}"
npm install --save \
...
export const useContribution = () => {
  ...
  const contribute = async (
    ...
    try {
      ...
      success(`Contributed ${amount} ${tokenType} successfully!`);
      return tx;
    } catch (err: any) {
...
export const useProfit = () => {
  ...
  const claimOrganizerProfit = async (eventPda: PublicKey) => {
    try {
      ...
      // Using SOL for organizer payouts
      const tx = await program.methods
...
export const BudgetVoting = ({ eventPda, event }: BudgetVotingProps) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [eventPda, publicKey, getContributor, getVote, getBudget]);
...
export const AddMilestoneForm = ({ eventPda, acceptedTokens, onSuccess }: AddMilestoneFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const milestoneIndex = Date.now(); // Simple unique index
...
export const TicketCheckIn = ({ eventPda }: TicketCheckInProps) => {
  const { checkInTicket, getTicket } = useTicket();
  ...
  const handleScan = async (result: any) => {
    if (result) {
      ...
      if (ticketData.ticketNumber) {
        await handleCheckIn(ticketData.ticketNumber);
      }
    }
  };
        ...
        <div>
          {!scanning ? (
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-gray-300 rounded-md text-gray-600 hover:border-blue-500 hover:text-blue-500"
            >
              📷 Scan QR Code
            </button>
          ) : (
...
export const ProfitClaim = ({ eventPda, event, isOrganizer }: ProfitClaimProps) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [eventPda, publicKey, isOrganizer, getProfitDistribution, getContributor, event.amountRaised]);
...
export const PlatformAdmin = () => {
  ...
  const handleWithdraw = async () => {
    setLoading(true);
    try {
      const amount = parseFloat(withdrawAmount) * 1e9; // Convert SOL to lamports
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchEvent();
  }, [params.address, publicKey, getEvent, isAdmin]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Contribution Form - Funding Phase */}
      {event.status === EventStatus.Funding && (
        <ContributionForm
          eventPda={eventPda!}
      ...
      {(event.status === EventStatus.Failed || event.status === EventStatus.Cancelled) && (
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {/* Refund component would go here */}
        </div>
      )}
    </div>
  );
...
# Continue from where the script was cut off

cat >> src/app/dashboard/page.tsx << 'EOL'
s grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {myTickets.map(({ pubkey, account }) => (
              <div key={pubkey.toBase58()} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p>Purchased: {new Date().toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
...
export default function MyTicketsPage() {
  ...
  useEffect(() => {
    ...
    fetchTickets();
  }, [publicKey, getUserTickets, getEvent]);
...
export default function MyContributionsPage() {
  ...
  useEffect(() => {
    ...
    fetchContributions();
  }, [publicKey, getProgramWithSigner, getEvent]);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {contributions.map(({ pubkey, account }) => {
                ...
                const canClaimProfit = event?.status === 'Completed' && 
                  account.profitsClaimed < (account.amount / event.amountRaised) * (event.profitDistribution?.backersShare || 0);
                ...
                return (
                  <tr key={pubkey.toBase58()}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      {isRefundable && !account.refundClaimed && (
...
# ============================================
echo -e "${GREEN}12. Creating EventCreationForm component...${NC}"
...
# ============================================
echo -e "${GREEN}13. Creating TypeScript type definitions...${NC}"
...
export const GATHERFI_ERRORS = {
  PlatformPaused: 6000,
  NotAuthorized: 6001,
  InvalidNigerianCity: 6002,
  InsufficientFunds: 6003,
  EventNotFound: 6004,
  InvalidEventStatus: 6005,
  FundingTargetNotReached: 6006,
  FundingDeadlinePassed: 6007,
  TicketAlreadyCheckedIn: 6008,
  TicketAlreadyRefunded: 6009,
  BudgetNotSubmitted: 6010,
  BudgetNotApproved: 6011,
  MaxTicketsExceeded: 6012,
  ProfitsAlreadyDistributed: 6013,
  NoProfitsToClaim: 6014,
  AlreadyVoted: 6015,
  InvalidTokenType: 6016,
  TokenNotAccepted: 6017,
  InvalidMint: 6018,
  InvalidTokenAccount: 6019,
  ProfitAlreadyClaimed: 6020,
  NoFeesToWithdraw: 6021,
  EventNotFailed: 6022
} as const;
EOL
...
# ============================================
echo -e "${GREEN}14. Updating global styles...${NC}"
...
/* Wallet Adapter Modal Customization */
...
# ============================================
echo -e "${GREEN}15. Updating Tailwind configuration...${NC}"
...
# ============================================
echo -e "${GREEN}16. Updating Next.js configuration...${NC}"
...
# ============================================
echo -e "${GREEN}17. Updating package.json scripts...${NC}"
...
# CREATE README.MD
# ============================================
echo -e "${GREEN}18. Creating README documentation...${NC}"
